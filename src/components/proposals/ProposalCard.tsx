...
interface ProposalCardProps {
  proposal: Proposal;
  projectId: string;
  onVote?: (proposalIndex: number, vote: boolean) => Promise<void>;
  canVote?: boolean;
  hasVoted?: boolean;
    ...
    <div className="bg-gray-700 rounded-lg shadow-lg p-6 mb-4">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm text-gray-400">
            <div>
              <p>Amount: <span className="text-white">{proposal.amount / 1e9} SOL</span></p>
      ...
      {!proposal.isExecuted && canVote && new Date(proposal.votingEnd * 1000) > new Date() && (
        <div className="flex space-x-4">
          <button
            onClick={() => onVote?.(proposal.milestoneId, true)}
            disabled={hasVoted}
          ...
          <button
            onClick={() => onVote?.(proposal.milestoneId, false)}
            disabled={hasVoted}
            className={`flex-1 px-4 py-2 rounded-lg transition-colors ${
              hasVoted && !userVote
      ...
      {!proposal.isExecuted && proposal.yesVotes > proposal.noVotes && new Date(proposal.votingEnd * 1000) < new Date() && (
        <button
          onClick={() => {/* Add release funds handler */}}
